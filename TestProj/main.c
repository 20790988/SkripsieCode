...
void copy_array_by_value(char *original, char *copy, uint32_t len, uint32_t num_copies);
void copy_pulse(uint32_t* original, uint32_t* copy, uint32_t len, uint32_t num_copies);
...
void copy_pulse(uint32_t* original, uint32_t* copy, uint32_t len, uint32_t num_copies)
{
    ...
    for (index_copy = 0; index_copy < PULSE_LENGTH/len; index_copy++)
    {
        ...
        {
            //printf("index copy %d index_segment %d\n",index_copy,index_segment);
            if (index_copy<num_copies)
            {
                printf("copy index %d to index %d \n",(index_segment),(index_copy*len+index_segment));
                copy[index_copy*len+index_segment] = original[index_segment];
                //printf("Copied %d to %d\n",original[index_segment],copy[index_copy*len+index_segment]);
            }
            else
            {
                printf("index = %d 0\n",(index_copy*len+index_segment));
                copy[index_copy*len+index_segment] = 0x0;
                //printf("Copied %d to %d\n",0,copy[index_copy*len+index_segment]);
            }
        }
    }
}
        ...
        uint32_t num_copies)
{
    uint32_t index_copy = 0;
    uint32_t index_segment = 0;
    for (index_copy = 0; index_copy < num_copies; index_copy++)
    {
        for (index_segment = 0; index_segment < len; index_segment++)
        {
            copy[index_copy*len+index_segment] = original[index_copy*len+index_segment];
            printf("Copied %d to %d",original[index_copy*len+index_segment],copy[index_copy*len+index_segment]);
        }
    }
